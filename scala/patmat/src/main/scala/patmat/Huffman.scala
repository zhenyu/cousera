...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    chars.map((k) => (k, 1)).groupBy(_._1).mapValues(_.size).toList
  }
  ...
  def singleton(trees: List[CodeTree]): Boolean = trees match {
    case Nil => false
    case _ :: Nil => true
    case current :: next => singleton(next) && current == next.head
  }

  def insertWithSort(codeTree: CodeTree, trees: List[CodeTree]): List[CodeTree] = trees match {
    case Nil => List(codeTree)
    case current :: rest =>
      if (weight(current) >= weight(codeTree)) List(codeTree) ++ trees else List(current) ++ insertWithSort(codeTree, rest)
  }
  ...
  /**
    ...
    */
  def until(stop: List[CodeTree] => Boolean, fold: (List[CodeTree]) => List[CodeTree])(origin: List[CodeTree]): List[CodeTree] = {
    if (!stop(origin)) until(stop, fold)(fold(origin))
    else origin
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    while (rest.size > 0) {
      val p = decodeOne(tree, rest)
      rest = p._2
      result = result ++ List(p._1)
    }
    result
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    for (elem <- chars(tree))
      yield (elem, encode(tree)(List(elem)))
  }
